...
{
    fastio
    int n,i,u,v,w,j,k; cin>>n; pp a[n+1];
    vector<pair<int,double>>adj[n+1];  
    vector<int> par(n+1,-1),vis(n+1,0);  vector<double> key(n+1,1000000007.900),ans;
    ...
    for(i=2;i<=n;i++) ans.pb(dist(a[i].F,a[i].S,a[par[i]].F,a[par[i]].S));
    sort(ans.begin(),ans.end());
    cout<<fixed<<setprecision(7)<<ans[n-k];
    return 0;
}